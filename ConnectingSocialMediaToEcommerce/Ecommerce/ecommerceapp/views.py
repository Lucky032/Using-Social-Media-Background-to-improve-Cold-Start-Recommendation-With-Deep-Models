...
from .forms import RegistrationForm, LoginForm, ProductForm, CommentForm, RatingForm
from .models import RegistrationModel, ProductModel, CommentModel, RatingModel, SearchHistoryModel, \
    TransactionModel
from .service import getAllProducts, findrecommendations, getProductById
...
def postProduct(request):
    ...
    if productForm.is_valid():
        ...
        tags=productForm.cleaned_data['tags']
        gender=productForm.cleaned_data['gender']
        age_group=productForm.cleaned_data['age_group']

        new_product = ProductModel(name=name,price=price,manufacturer=manufacturer,category=category,
                                   description=description,path=path,tags=tags,gender=gender,age_group=age_group)
...
def buyProductAction(request):
    ...
    transaction=TransactionModel(userid=request.session['username'],productid=request.GET['product'],productname=product.name,status="pending")
    transaction.save()
...
def postrating(request):
    return render(request, "rateproduct.html", {"productbean": getProductById(request.GET['pid']),
                                             "userrecommendations": findrecommendations(request.session['username'])})